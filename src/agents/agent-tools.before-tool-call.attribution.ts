...
export function resolveToolExecutionCorrelation(ctx?: HookContext): ToolExecutionCorrelation {
  ...
  const agentId = attribution?.agentId ?? ctx?.agentId;
  const sessionKey = attribution?.sessionKey ?? ctx?.sessionKey;
  const sessionId = attribution?.sessionId ?? ctx?.sessionId;
  const runId = attribution?.runId ?? ctx?.runId;
  return {
    ...(agentId ? { agentId } : {}),
