...
export function rememberRunTerminal(
  ...
  const authoritativeToken = getAuthoritativeRunContextToken(runInstance, runId);
  const remembered =
    (authoritativeToken ? state.authoritativeOpenProvenance.get(authoritativeToken) : undefined) ??
    state.openRunProvenance.get(runInstance) ??
    state.runProvenance.get(runInstance) ??
    provenance;
  if (authoritativeToken) {
...
export function forgetAuthoritativeOpenRun(
  ...
  const token = getAuthoritativeRunContextToken(runInstance, runId);
  if (token) {
...
export function hasAuthoritativeRunContext(runInstance: string, runId: string): boolean {
  return getAuthoritativeRunContextToken(runInstance, runId) !== undefined;
}
